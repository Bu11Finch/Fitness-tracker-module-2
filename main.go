...
func (r Running) Calories() float64 {
	...
	return ((CaloriesMeanSpeedMultiplier*meanSpeed + CaloriesMeanSpeedShift) * weight / MInKm * duration * MinInHours)
}
...
func (w Walking) Calories() float64 {
	...
	meenSpeedMSec := w.Training.meanSpeed() * MInKm / 3600
	duration := w.Training.Duration.Hours()
	return ((CaloriesWeightMultiplier*weight + (math.Pow(meenSpeedMSec, 2)/w.Height)*CaloriesSpeedHeightMultiplier*weight) * duration * MinInHours)
}
...
func (s Swimming) meanSpeed() float64 {
	...
	return float64(s.LengthPool) * float64(s.CountPool) / MInKm / duration
}
...
func (s Swimming) Calories() float64 {
	...
	duration := s.Training.Duration.Hours()
	return (s.meanSpeed() + SwimmingCaloriesMeanSpeedShift) * SwimmingCaloriesWeightMultiplier * weight * duration
}
